...
class EloquentExchangeRateRepository implements ExchangeRateRepositoryInterface
{
    ...
    public function trendData(ExchangeRate $exchangeRate)
    {
        return [
            ...
            '7_0_min' => $this->trackTrend($exchangeRate, time()-(7*60), time()),
            '7_3_min' => $this->trackTrend($exchangeRate, time()-(7*60), time()-(3*60)),
            '3_0_min' => $this->trackTrend($exchangeRate, time()-(3*60), time()),
            '2_0_min' => $this->trackTrend($exchangeRate, time()-(2*60), time()),
        ];
    }
